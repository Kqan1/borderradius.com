"use client"
import { Dispatch, SetStateAction, forwardRef, LinkHTMLAttributes, useRef, useEffect } from "react";
...
export interface DropdownProps extends LinkHTMLAttributes<HTMLAnchorElement>, VariantProps<typeof DropdownVariants> {
...
const Dropdown = forwardRef<HTMLDivElement, DropdownProps>(
    ({ className, children, links, variant, size, state, setState, ...props }, ref) => {
                ...
                <div className="divide-none divide-y-2 divide-zinc-300 lg:divide-solid dark:divide-zinc-500">
                    {links.map((linkGroup: any, index: any) => (
                        <div className="flex flex-col p-0 lg:py-1" key={index}>
                            {linkGroup.map((link: Link, linkIndex: number) => (
                                    <Link
                                    key={linkIndex}
                                    href={link.href ?? "/"}
